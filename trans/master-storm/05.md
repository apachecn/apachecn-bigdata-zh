...
在上一章中，我们对Trident进行了概述。 在本章中，我们将介绍Trident拓扑的发展。 以下是我们将在本章中介绍的重要要点：
...
本节解释如何编写一个Trident hello world 拓扑。 执行以下步骤创建Trident hello world 拓扑：
...
3.  在`com.stormadvance.storm_trident`包中创建`TridentUtility`类。 此类包含我们将在Trident hello world 示例中使用的Trident过滤器和函数：
...
4.  在`com.stormadvance.storm_trident`包中创建`TridentHelloWorldTopology`类。 此类定义 hello world Trident拓扑：
...
让我们逐行理解代码。 首先，我们创建了一个`TridentTopology`类的对象，用于定义Trident计算。

`TridentTopology`包含一个名为`newStream()`的方法，该方法将接受一个输入源作为参数。 在本例中，我们使用在非事务性拓扑部分中创建的`FakeTweetSpout`作为输入源。 与 Storm 一样，Trident还维护 ZooKeeper 中每个输入源的状态。 这里，`FakeTweetSpout`字符串指定了 ZooKeeper 中维护元数据的节点。
...
分布式 RPC 用于动态查询和检索Trident拓扑的结果。 Storm 有一个内置的分布式 RPC 服务器。 分布式 RPC 服务器从客户端接收 RPC 请求，并将其传递到 Storm 拓扑。 拓扑处理请求并将结果发送到分布式 RPC 服务器，该服务器由分布式 RPC 服务器重定向到客户端。
...
让我们假设我们正在数据库中存储 hello world Trident拓扑的计数聚合，并希望动态检索给定国家/地区的计数。 我们需要使用分布式 RPC 功能来实现这一点。 此示例显示了如何将分布式 RPC 合并到上一节创建的 hello world Trident拓扑中：
...
我们使用`FakeTweetSpout`作为输入源，使用`TridentTopology`类定义Trident计算。
...
`persistentAggregate`将流转换为Trident`State`对象。 在本例中，Trident`State`对象表示到目前为止每个国家的计数。
...
`Split`函数用于分割逗号分隔的国家/地区列表。 我们使用了`stateQuery()`方法来查询拓扑的第一部分中定义的Trident`State`对象。 `stateQuery()`接受状态源--在本例中，是由拓扑的第一部分和用于查询该函数的函数计算的国家计数。 我们使用的是`MapGet()`函数，该函数获取每个国家的计数。 最后，返回每个国家/地区的计数作为查询输出。
...
Trident分布式 RPC 查询的执行方式与普通 RPC 查询类似，只是这些查询是并行运行的。
...
使用Trident拓扑和Trident拓扑很容易实现只需一次的处理。 另一方面，在香草Storm的情况下，很难实现精确的一次处理。 因此，对于我们只需要一次处理用例，Trident将非常有用。

Trident并不适合所有的用例，尤其是高性能的用例，因为Trident增加了 Storm 的复杂性并管理状态。

# 简略的 / 概括的 / 简易判罪的 / 简易的

在本章中，我们主要关注Trident样本拓扑、Trident`groupBy`操作和非事务拓扑。 我们还介绍了如何使用分布式 RPC 在Trident拓扑上进行动态查询。
