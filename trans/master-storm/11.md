...
现在，实时日志数据正在进入Kafka话题。 在下一节中，我们将编写 Storm 拓扑来使用日志数据、处理流程数据并将其存储到数据库中。

# 为什么我们要在 Logstash 和 Storm 之间使用Kafka？
...
本节介绍如何根据推荐人 URL 计算搜索关键字。 假设引用 URL 为[https://www.google.co.in/#q=learning+storm](https://www.google.co.in/#q=learning+storm)。 我们将把这个引用 URL 传递给一个类，类的输出将是*学习Storm*。 通过识别搜索关键字，我们可以轻松识别用户为访问我们的站点而搜索的关键字。 让我们执行以下步骤，从 Referrer URL 计算关键字：
...
本节介绍了如何将不同类型的螺栓链接到拓扑中。 此外，我们还介绍了如何使用Kafka的数据。 在下一节中，我们将说明如何部署拓扑。
