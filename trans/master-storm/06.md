...
在前面的章节中，我们介绍了 Storm 的基础知识、Storm 的安装、Storm 的开发和部署，以及 Storm 集群中的Trident拓扑。 在本章中，我们将重点介绍 Storm Schedulers。
...
让我们考虑一个例子。 假设我们有一个拓扑，它有一个管口和两个螺栓，每个组件都有一个执行器和一个任务。 下图显示了如果我们将拓扑提交到 Storm 群集，拓扑的分布情况。 假设分配给拓扑的工作进程数为 3，Storm群集中的主控引擎数为 3：
