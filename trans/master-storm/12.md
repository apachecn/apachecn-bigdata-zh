...
*   从 Twitter 上收集推文并将其存储在Kafka(Kafka)中
*   从Kafka读取数据，计算情感，并将其存储在 HDFS 中
...
我们向您介绍了 Storm 的基本概念和组件，并介绍了如何在本地和集群模式下编写和部署/运行拓扑。 我们还演练了 Storm 的基本命令，并介绍了如何在运行时修改 Storm 拓扑的并行性。 我们还专门用了整整一章来监测 Storm，这是一个在开发过程中经常被忽视的领域，但它是任何生产环境中的关键部分。 您还了解了Trident，它是低级 Storm API 的抽象，可用于开发更复杂的拓扑和维护应用程序状态。
