...
在运行时更新拓扑并行性的过程称为**重新平衡**。 有关此操作的更详细信息，请参见[第 3 章](03.html#1LCVG0-6149dd15e07b443593cc93f2eb31ee7b)，*Storm并行性和数据分区*。

# 杀死 / 消磨 / 使终止

Storm拓扑是永无止境的过程。 要停止拓扑，我们需要终止它。 取消时，拓扑首先进入停用状态，处理已发送到其中的所有元组，然后停止。 运行以下命令终止`SampleStormClusterTopology`：
