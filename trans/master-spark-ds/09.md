...
如前所述，确保Spark转换中使用的大型集合已经广播给所有Spark执行器。这降低了与网络传输相关的成本。
...
通过这种方法，我们将文本[掌握数据科学Spark- V1]转换为[掌握Spark数据科学]，从而减少输入向量的字数(因此减少维数)。最后，我们使用 MLlib `normalizer`类来规范化我们的词频向量:
...
因为Spark流在后台使用`twitter4j`，所以配置是使用`twitter4j`库中的`ConfigurationBuilder`对象完成的:
