...
RBAC 是通过 Apache Sentry 在 Hadoop 中实现的。组织可以定义数据集的权限，这些权限将从多个访问路径强制实施，包括 HDFS、Apache蜂巢、英帕拉，以及通过 HCatalog 的Apache Pig 和Apache MapReduce/shate。例如，每个 Spark 应用都作为请求用户运行，并要求访问底层文件。Spark 不能直接强制访问控制，因为它是作为请求用户运行的，不受信任。因此，它仅限于文件系统权限(ACL)。在这种情况下，Apache Sentry 为资源提供基于角色的控制。
