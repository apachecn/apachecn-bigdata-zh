...
它使用`SparkContext()`的`textFile()`方法，并返回包含一组行的字符串的 RDD。在[第 1 章](01.html#DB7S2-0b803698e2de424b8aa3c56ad52b005d "Chapter 1. Introduction to Data Analytics with Spark")、*使用 Spark* 介绍数据分析时，我们解释了什么是 SparkContext。然而，Spark上下文是Spark应用的入口点。假设我们有一个名为`1.txt`的数据集，其中包含一些作为非结构化文本的推文数据。您可以从 Packt 材料下载数据并存储在`project_path/input/test/`目录下，定义如下:
