...
![Creating and building the model](../images/00142.jpeg)
...
![Data exploration](../images/00123.jpeg)
...
![Data exploration](../images/00065.jpeg)
...
![Movie recommendation using Spark MLlib](../images/00108.jpeg)
...
![Movie recommendation using Spark MLlib](../images/00135.jpeg)
...
![Movie recommendation using Spark MLlib](../images/00146.jpeg)
...
![Movie recommendation using Spark MLlib](../images/00145.jpeg)
...
![Movie recommendation using Spark MLlib](../images/00076.jpeg)
...
![Developing a real-time ML pipeline from streaming](../images/00129.jpeg)
...
请注意，在 Spark 从 1.6.2 升级到 2.0.0 期间，Twitter(以及其他一些)驱动程序支持已被删除。这意味着使用 Spark 从较少使用的流连接器(包括 Twitter、Akka、MQTT 和 ZeroMQ)收集流数据的支持在 Spark 2.0.0 中已被删除。因此，不可能使用 Spark 2.0.0 开发一个用于推特数据收集的应用程序。因此，火花 1.6.1 将用于本节中的推特数据收集演示。建议读者使用提供的 Maven 友好`pom.xml`文件在 Eclipse 上创建一个 Maven 项目。
...
![Topic modeling using Spark](../images/00042.jpeg)
...
![Topic modeling using Spark](../images/00098.jpeg)
...
在下一节中，我们将介绍如何使用 Spark 的 GraphX API 解析和处理大规模的图形数据，以便从我们从*图 10* 获得的主题数据中找到连接的组件。
...
然后火花外壳将与火花会话一起提供。我们假设您的火花分布保存在`home/spark-2.0.0-bin-hadoop2.7`路径中。请相应地更改路径，以便运行火花壳。另外，请将*图 11* 中显示的主题术语保存到单独的文本文件中，以便在继续执行下一步之前，您可以将这些术语用作图形数据进行分析:
...
![Getting and parsing graph data using the GraphX API](../images/00126.jpeg)
...
![Finding the connected components](../images/00085.jpeg)
