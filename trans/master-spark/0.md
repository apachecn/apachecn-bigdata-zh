...
接下来，介绍配置单元上下文。 最初，创建本地上下文，然后对其执行配置单元 QL 操作。 然后，介绍了一种将现有的分布式 CDH 5.3 配置单元集成到Spark配置单元上下文的方法。 然后显示针对此上下文的操作，以更新群集上的配置单元数据库。 通过这种方式，可以创建和调度 Spark 应用，以便由实时 Spark 引擎驱动配置单元操作。
