...
下面的图借用自 spk.apache.org网站，演示了 Apache Spark 集群管理器在主应用、从应用(辅助应用)、执行器应用和 Spark 客户端应用中的角色：
...
通过指定 Spark 配置本地 URL，可以在本地运行应用。 通过指定local[n]，可以让 Spark 使用`<n>`线程在本地运行应用。 这是一个有用的开发和测试选项。
