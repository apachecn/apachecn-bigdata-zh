...
目前，Flink 允许使用 CSV 源从 CSV 文件中读取数据，并使用Kafka源从Kafka主题中读取 JSON 数据。
...
可以使用`SELECT STREAM`关键字对注册了`TableEnvironment`的数据流执行 SQL 查询。大多数 SQL 语法在数据集和数据流之间是通用的。要了解更多关于流语法的知识，Apache方解石的流文档会很有帮助。可在:[https://calcite.apache.org/docs/stream.html](https://calcite.apache.org/docs/stream.html)找到。
