...
Flink流媒体应用编程接口从谷歌数据流模型中获得灵感。它的流式应用编程接口支持不同的时间概念。总的来说，在流媒体环境中，我们有三个地方可以捕捉时间。它们如下
...
Kafka 是一个发布-订阅、分布式的消息队列系统，允许用户发布某个主题的消息；然后分发给该主题的订阅者。Flink提供了在Flink流中定义Kafka消费者作为数据源的选项。为了使用 Flink Kafka 连接器，我们需要使用一个特定的 JAR 文件。

下图显示了 Flink Kafka连接器的工作原理:

![Kafka connector](img/image_02_004.jpg)

我们需要使用以下 Maven 依赖项来使用连接器。我一直在使用Kafka 0.9 版本，所以我将在`pom.xml`中添加以下依赖项:
...
现在让我们试着理解如何使用Kafka消费者作为Kafka的来源:
...
为了启用容错，我们需要在 Flink 中启用检查点。Flink热衷于定期拍摄该州的快照。如果出现故障，它将恢复到最后一个检查点，然后重新开始处理。

我们也可以把Kafka的生产者定义为一个水槽。这将把数据写入一个Kafka式的主题。以下是将数据写入Kafka主题的一种方式:
...
在许多用例中，您可能希望使用 Flink 处理数据，然后将其存储在弹性搜索中。为此，Flink 支持弹性搜索连接器。到目前为止，弹性搜索已经发布了两个主要版本。Flink支持他们两个。
...
Cassandra 是一个分布式、低延迟的 NoSQL 数据库。这是一个基于键值的数据库。许多高吞吐量应用程序使用 Cassandra 作为它们的主数据库。Cassandra 采用分布式集群模式，没有主从架构。任何节点都可以支持读写。关于Cassandra的更多信息，请访问:http://cassandra.apache.org/。

Apache Flink 提供了一个可以向 Cassandra 写入数据的连接器。在许多应用程序中，人们可能希望将 Flink 的流数据存储到 Cassandra 中。下图显示了Cassandra水槽的简单设计:
...
在这个场景中，我们假设传感器正在向名为 **temp** 的Kafka主题发送信息，信息为(时间戳、温度、传感器-ID)。现在我们需要编写代码来读取Kafka主题中的数据，并使用 Flink 转换对其进行处理。

这里需要考虑的重要事情是，由于我们已经有了来自传感器的时间戳值，我们可以使用事件时间计算时间因素。这意味着我们将能够处理事件，即使它们超出了正常范围。

我们从简单的流执行环境开始，它将从Kafka那里读取数据。由于事件中有时间戳，我们将编写一个自定义时间戳和水印提取器来读取时间戳值，并基于此进行窗口处理。下面是相同的代码片段。
...
这里我们假设我们以字符串的形式接收Kafka主题中的事件，格式如下:
...
当执行前面给定的代码时，如果在Kafka主题上发布了适当的传感器事件，那么我们将每五分钟获得每个传感器的平均温度。
