...
我们应用程序的架构相对简单。 我们将创建一个 Twitter 客户端应用程序，该应用程序读取 Twitter Firehose 的子集，并将每条消息作为 JSON 数据结构写入 Kafka 队列。 然后，我们将使用Kafka喷嘴将这些数据输入到我们的Storm拓扑中。 最后，我们的 Storm 拓扑将分析传入的消息并填充图形数据库。
