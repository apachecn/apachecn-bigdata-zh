...
*Storm蓝图：分布式实时计算的模式*涵盖了广泛的分布式计算主题，不仅包括设计和集成模式，还包括该技术立即有用和广泛应用的领域和应用。 本书使用真实世界的示例向读者介绍 Storm，从简单的 Storm 拓扑开始。 这些示例增加了复杂性，引入了先进的 Storm 概念以及更复杂的部署和运营问题方法。
...
*Storm蓝图：分布式实时计算的模式*通过描述基于真实世界示例应用的广泛适用的分布式计算模式，使初学者和高级用户都受益。 这本书介绍了Storm和Trident的核心原语，以及成功部署和操作所需的关键技术。
