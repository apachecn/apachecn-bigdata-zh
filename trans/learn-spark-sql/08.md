...
Spark数据帧也可以注册为Spark SQL 中的临时视图，允许我们对其数据运行 SQL 查询。`sql`函数使应用程序能够以编程方式运行 SQL 查询，并将结果作为Spark数据帧返回。
...
在下一章中，我们将使用 Spark 模块的混合来构建 Spark 应用程序。我们将展示将Spark SQL 与Spark流、Spark机器学习等相结合的应用程序示例。