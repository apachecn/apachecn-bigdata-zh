...
SparkR 的分布式数据框架支持 R 用户熟悉的编程语法。高级数据框架应用编程接口将 R 应用编程接口与Spark中优化的 SQL 执行引擎集成在一起。
...
`read.df`方法可用于从各种数据源创建Spark数据帧。我们需要指定输入数据文件的路径和数据源的类型。数据源应用编程接口本身支持格式，如 CSV、JSON 和 Parquet。
...
数据源应用编程接口可用于将Spark数据帧保存为多种不同的文件格式。例如，我们可以使用`write.df`将上一步创建的Spark数据帧保存到拼花文件中:
...
首先，我们使用缓存或持久函数来缓存Spark数据帧以获得性能。我们还可以指定存储级别选项，如`DISK_ONLY`、`MEMORY_ONLY`、`MEMORY_AND_DISK`等，如下图所示:
...
接下来，我们从目前为止使用的分布式 Spark 版本创建一个本地数据帧。我们使用`collect`功能将Spark数据帧移动到Spark驱动器上的本地/R 数据帧，如图所示。通常，您会在将数据移动到本地数据框之前对其进行汇总或取样:
...
Spark数据帧也可以注册为Spark SQL 中的临时视图，允许我们对其数据运行 SQL 查询。`sql`函数使应用能够以编程方式运行 SQL 查询，并将结果作为Spark数据帧返回。

首先，我们将Spark数据框注册为临时视图:
...
在 SparkR 中，支持多种类型的**用户自定义功能** ( **UFDs** )。例如，我们可以使用`dapply`或`dapplyCollect`在大数据集上运行给定的函数。`dapply`函数对Spark数据帧的每个分区应用一个函数。函数的输出应该是 data.frame。

该模式指定生成的Spark数据帧的行格式:
...
`gapplyCollect`类似地对Spark数据帧的每个分区应用一个函数，但也将结果收集回`R data.frame`。
...
在下一章中，我们将使用 Spark 模块的混合来构建 Spark 应用。我们将展示将Spark SQL 与Spark流、Spark机器学习等相结合的应用示例。