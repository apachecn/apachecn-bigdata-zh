 ...
 *在过去的几年里，Scala 一直在稳步上升，并被开发人员和从业者广泛采用，尤其是在数据科学和分析领域。另一方面，ApacheSpark是；用 Scala 编写的是一个快速通用的大规模数据处理引擎。Spark 的成功得益于很多因素:易用的 API、干净的编程模型、性能等等。因此，Spark 自然对 Scala 有了更多的支持:与 Python 或 Java 相比，Scala 有更多的 API 可用；尽管如此，在 Java、Python 和 r 的 API 之前，已经有了新的 Scala APIs。
