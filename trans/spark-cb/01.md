...
Yar遵循主从体系结构。 主守护进程称为`ResourceManager`，从守护进程称为`NodeManager`。 除了此应用，生命周期管理由`ApplicationMaster`完成，它可以在任何从节点上派生，并且在应用的生命周期内都是有效的。
...
Spark RDDS是在内存中存储数据集的一种很好的方式，同时在不同的应用中得到相同数据的多个副本。 Tachyon 使用 Spark RDD 管理解决了一些挑战。 其中几个是：
