...
构建数据密集型应用程序不像将数据库暴露在 Web 界面上那么简单。 在设置静态数据和运动处理数据的过程中，我们将利用 Spark 的交互分析数据的能力，优化机器学习和流媒体活动所需的数据丰富性和质量。 在这里，我们将经历一个数据收集、提炼和调查的迭代周期，以获得我们的应用程序感兴趣的数据集。
