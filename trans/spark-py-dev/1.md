...
Hadoop随着数据的增长进行水平扩展。 Hadoop 在商用硬件上运行，因此性价比高。 可扩展的分布式处理框架支持密集型数据应用，使组织能够分析大型商用群集上的 PB 级数据。 Hadoop 是 Map-Reduce 的第一个开源实现。 Hadoop依赖于称为**HDFS**(**Hadoop Distributed File System**)的分布式存储框架。 Hadoop 在批处理作业中运行 map-duce 任务。 Hadoop 要求在每个映射、洗牌和还原过程步骤中将数据持久化到磁盘。 此类批处理作业的开销和延迟会对性能产生不利影响。
...
SPARK应用由一个驱动程序程序组成，该程序运行用户的主函数，在集群上创建分布式数据集，并对这些数据集执行各种并行操作(转换和操作)。
...
Spark在 JVM 上运行，需要Java**SDK**(缩写为**Software Development Kit**)，而不是**JRE**(**Java Runtime Environment**)，因为我们将使用 Spark 构建应用。 推荐的版本是 Java 版本7 或更高版本。 Java8 是最合适的，因为它包含了 Scala 和 Python 提供的许多函数式编程技术。
