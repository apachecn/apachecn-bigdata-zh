...
![Understanding machine learning](graphics/image_07_001.jpg)
...
*   **转换器**:这将转换包含特征的火花数据帧，并将其转换为另一个包含预测的火花数据帧。估计器创建的模型是一个转换器。
...
![Wine quality prediction](graphics/image_07_002.jpg)
...
二十个新闻组数据集下载链接允许您下载这里讨论的数据集。文件`20_newsgroups.tar.gz`要下载并解压。以下代码片段中使用的数据目录应该指向以解压缩形式提供数据的目录。如果火花驱动程序由于数据量大而出现内存不足错误，请删除一些不感兴趣的新闻组数据，并尝试使用数据的子集。这里，为了训练模型，只使用了以下新闻组数据:talk.politics.guns，talk.politics.mideast，talk.politics.misc，和 talk .宗教学. misc。
...
Scala 实现和 Python 实现的主要区别在于，在 Python 实现中，停止字没有被删除。这是因为该功能在火花机器库的 Python 应用编程接口中不可用。由于这种差异，Scala 程序和 Python 程序生成的同义词列表是不同的。
