...
在图形框架的下面是两个Spark数据框架:一个用于顶点，另一个用于边缘。GraphFrames 可能被认为是 Spark 的 GraphX 库的下一代，对后者进行了一些重大改进:
...
确保您已经从前面的小节创建了`graph`图形框架和源`deptsDelays_GEO`数据框架。
