...
*   分类:
    *   `LinearSVC`是线性可分问题的 SVM 模型。SVM 核具有![](../images/00136.jpeg)形式(超平面)，其中![](../images/00137.jpeg)是系数(或超平面的法向量)![](../images/00138.jpeg)是记录， *b* 是偏移量。
    ...
    *   `NaiveBayes`使用贝叶斯条件概率理论![](../images/00139.jpeg)，根据证据和关于概率和似然性的先验假设对观测值进行分类。
...
*   `impurity`:默认设置为`variance`，目前(在火花 2.3 中)是唯一允许的选项
...
要执行这个食谱，你需要一个工作的火花环境。你还必须执行之前的食谱。
