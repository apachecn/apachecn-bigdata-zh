...
5.  去Apache星火网站([https://spark.apache.org/](https://spark.apache.org/))。
6.  要下载Spark，请从下拉菜单中选择以下选项:
    *   最近发布的Spark
    *   合适的包装类型
...
Spark上下文是Spark中的一个对象或概念。这是一个大数据分析引擎，允许您以编程方式利用 Spark 的力量。
...
让我们考虑一个例子，我们有一个有 5000 万个名字的大型联系人数据库，我们可能想从这些联系人中提取第一个名字。显然，很难将 5000 万个名字放入您的本地内存中，尤其是如果每个名字都嵌入在一个更大的联系人对象中。这就是Spark出现的地方。Spark 允许你给它一个大数据文件，并将帮助处理和上传这个数据文件，同时为你处理对这个数据进行的所有操作。该电源由 Spark 的集群管理器管理，如下图所示:
...
我们可以看到文本文件中的`20`行包含`Spark`这个词。这只是我们如何使用Spark壳的一个简单例子。
