...
另外，标签可以是任何类型——它不需要是`String`类型；我们可以键入我们想要的内容并传递它。一旦我们有了顶点、用户和边的关系，我们就可以创建一个图。我们正在使用`Graph`类的`apply`方法来构建我们的Spark图。我们需要通过`users`、`VertexId`和`relationships`，如下:
