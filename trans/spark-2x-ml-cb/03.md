...
4.  导入火花会话所需的包，以访问集群并`Log4j.Logger`减少火花产生的输出量。
...
6.  将输出电平设置为`ERROR`以减少火花的输出。

```scala
Logger.getLogger("org").setLevel(Level.ERROR)Logger.getLogger("akka").setLevel(Level.ERROR)
```

7.  创建一个火花会话，允许访问火花集群和基础会话对象属性，如火花上下文和火花上下文。
