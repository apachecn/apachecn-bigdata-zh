...
运行时效率考虑:`groupBy()`功能通过按键对数据进行分组。该操作导致内部洗牌，这可能会爆炸执行时间；人们必须总是更喜欢使用`reduceByKey()`操作家族来直接进行`groupBy()`方法调用。`groupBy()`方法由于洗牌是一个昂贵的操作。每个组都由键和属于该键的项组成。Spark 不会保证与密钥对应的值的顺序。
...
7.  创建一个 Spark 会话，允许访问 Spark 集群和基础会话对象属性，如 Spark 上下文和 Spark 上下文。
