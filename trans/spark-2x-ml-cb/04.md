...
6.  使用构建器模式初始化指定配置的 Spark 会话，从而为 Spark 集群提供入口点:
...
我们读入文件并解析文件中的数据。数据集对象由 Spark 创建。我们在控制台中确认类型，然后显示数据:
...
我们可以通过 Spark 会话直接创建数据框和数据集。它可以工作，但是在 Spark 2.0.0 中被标记为实验性的。

如果您要做任何与 SQL 相关的工作，SparkSession 现在是 Spark SQL 的入口点。Spark 会话是创建 Spark SQL 应用必须创建的第一个对象。
...
7.  导入 Spark 会话所需的包以访问集群并`log4j.Logger`减少 Spark 产生的输出量:
...
9.  通过使用构建器模式指定配置来初始化 Spark 会话，从而为 Spark 集群提供入口点:
