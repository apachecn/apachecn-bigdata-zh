...
8.  初始化指定配置的`SparkSession`以访问Spark簇:
...
6.  使用构建器模式初始化指定配置的`SparkSession`，从而为Spark簇提供入口点:
...
最后，我们在控制台中显示模型边界和预测，并使用模型转换测试数据集，在控制台中显示结果数据框，其中包含预测字段。所有Spark最大似然算法对超参数值敏感。虽然设置这些参数没有硬性规定，但在投入生产之前，需要使用科学方法进行大量实验。
...
6.  初始化指定配置的`SparkSession`以访问Spark簇:
...
6.  初始化一个`SparkSession`指定构造Spark簇入口点的配置:
...
6.  使用构建器模式初始化指定配置的`SparkSession`，从而为Spark簇提供一个入口点:
...
生存回归的Spark实现(`AFTSurvivalRegression` ) *:*
