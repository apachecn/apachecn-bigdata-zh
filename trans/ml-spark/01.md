...
*   使用 Amazon Elastic Map Reduce 设置Spark集群
...
*   有关Spark快速入门的信息，请参阅[http://spark.apache.org/docs/latest/quick-start](http://spark.apache.org/docs/latest/quick-start)
...
它是 Spark 功能的主要入口点。 `SparkContext`是到Spark星团的连接。 它可用于在群集上创建 RDDS、累加器和广播变量。
...
Spark操作在风格上是功能性的。 对于熟悉 Java8 中 Scala、Python 或 Lambda 表达式的函数式编程的程序员来说，这些操作应该很自然。 对于那些没有函数式编程经验的人来说，不用担心；Spark API 相对容易学习。
...
Spark 的 Python API 几乎用 Python 语言公开了 Spark 的 Scala API 的所有功能。 有些功能还不受支持(例如，使用 GraphX 和一些 API 方法进行图形处理)。 有关详细信息，请参阅*Spark编程指南*([http://spark.apache.org/docs/latest/programming-guide.html](http://spark.apache.org/docs/latest/programming-guide.html))的 Python 节。
...
可以使用`./bin/sparkR shell`运行Spark分配。
...
在接下来的步骤中，我们将使用 Google Cloud Console(用于创建Spark集群和提交作业的用户界面)。

# 创建群集

您可以通过云平台控制台创建Spark集群。 选择项目，然后单击继续以打开群集页面。 您将看到属于您的项目的 Cloud DataProc 集群(如果您已经创建了任何集群)。
...
配置完页面上的所有字段后，单击 Create(创建)按钮创建群集。 创建的集群名称将显示在集群页面上。 一旦创建Spark群集，状态就会更新为 Running。
