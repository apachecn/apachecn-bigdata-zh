...
Spark Streaming 采用第二种方法。 火花流中的核心原语是**离散化的流**或**DStream**。 DStream 是一系列小批次，其中每个小批次表示为 Spark RDD：

![](images/image_11_001.png)
...
火花流**接收器**负责从**输入源**接收数据，并将原始数据转换成由 Spark RDDS 组成的 DStream。
...
火花流转换(例如那些适用于 RDDS 的转换)对 DStream 底层数据的每个元素进行操作。 也就是说，转换被有效地应用于 DStream 中的每个 RDD，而 DStream 中的每个 RDD 又将转换应用于 RDD 的元素。
...
火花流还支持在发生故障时恢复驱动程序节点。 然而，目前基于网络的源在这种情况下会丢失工作节点内存中的数据。 因此，火花流在驱动程序节点或应用程序出现故障时不能完全容错。 相反，这里可以使用 lambda 架构。 例如，夜间批处理可以在失败的情况下完成并纠正问题。
