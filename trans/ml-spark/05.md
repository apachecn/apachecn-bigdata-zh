...
*q*和*p*之间的点积![](images/image_6.png)捕捉了用户 u 和项目 i 之间的交互，即用户对项目的兴趣。 建模的关键是找到向量*q*和*p*。

在设计模型时，首先要得到用户与物品之间的潜在关系。 生成评级矩阵的低维表示形式。 对评级矩阵进行奇异值分解，得到*Q*，*S*，*P*。 将矩阵*S*降维为维数*k*，得到*q*和*p*。

**![](images/Screen-Shot-2017-04-27-at-3.55.57-PM.png)**

现在，计算一下建议：

![](images/Screen-Shot-2017-04-27-at-3.56.04-PM.png)

优化函数(在观察到的评级上)如下图所示；学习潜在因子向量*q*和*p*，系统会最小化评级集上的正则化平方误差。

![](images/Screen-Shot-2017-04-27-at-3.56.08-PM.png)
...
在线性代数中，向量![](images/image_05_014.png)的大小称为![](images/image_05_015.png)的范数。 我们将讨论几种不同的规范。 在本讨论中，我们将向量 v 定义为有序的数字元组。

![](images/image_05_016.png)

One Norm: The one-norm (also known as the L1-norm, or mean norm) of vector ![](images/image_05_017.png) is denoted as shown in the following diagram and is defined as the sum of the absolute values of its components:

![](images/image_05_018.png)

Two-norm (also known as the L2-norm, mean-square norm, least-squares norm)
of a ![](images/image_05_019.png) vector is denoted as shown in this diagram::

![](images/image_05_020.png)

Moreover, it is defined as the square root of the sum of the squares of the absolute values of its components:

![](images/image_05_021.png)

这样，余弦相似度就是归一化的点积。 余弦相似性度量采用介于`-1`和 1 之间的值。值`1`表示完全相似，而值 0 表示独立(即没有相似性)。 此度量非常有用，因为它还捕获负相似性，也就是说，值`-1`意味着不仅向量不相似，而且它们也完全不相似：

![](images/image_05_022.png)
...
我们将找出最频繁的项目(在本例中为字符)。 首先，我们将获得火花上下文，如下所示：
